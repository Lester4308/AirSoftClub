...
namespace AirsoftClub.Unity
{
    public sealed partial class ClubClient
    {
        ...
        void RenderRecruitmentModern(RectTransform root)
        {
            ...
            ((RectTransform)root.GetChild(root.childCount - 1)).anchoredPosition = new Vector2(CX, MY - 95);

            // Grid of candidates
            for (int n = 0; n < club.Offers.Length; n++)
            {
                var o = club.Offers[n];
                var card = PanelRect("Offer" + n, root, CX + n % 4 * 330, CY + 20 + n / 4 * 260, 310, 240);
                card.gameObject.AddComponent<Image>().color = o.Id == selectedOffer ? ModernStyle.Neutral : ModernStyle.Card;
                card.GetComponent<Image>().raycastTarget = false;
                MButton("INSPECT", card, 0, -30, 140, 40, () => selectedOffer = o.Id, ModernStyle.Blue);
                MLabel(o.Name, card, 17, ModernStyle.Ink, TextAnchor.MiddleLeft);
                ((RectTransform)card.GetChild(0)).sizeDelta = new Vector2(280, 30); ((RectTransform)card.GetChild(0)).anchoredPosition = new Vector2(0, -90);
                MLabel("A " + o.Accuracy + "   E " + o.Endurance + "   G " + o.Agility, card, 14, ModernStyle.Muted, TextAnchor.MiddleLeft);
                ((RectTransform)card.GetChild(1)).sizeDelta = new Vector2(280, 28); ((RectTransform)card.GetChild(1)).anchoredPosition = new Vector2(0, -150);
            }
            ...
            var det = PanelRect("Detail", root, CX + 660, CY + 20, 640, 330);
        ...
        void RenderShopModern(RectTransform root)
        {
            ...
            {
                var cat = cats[n];
                var cbtn = MButton(cat, root, CX, CY + 40 + n * 55, 180, 44, () => { shopCategory = cat; catalogScroll = Vector2.zero; }, shopCategory == cat ? ModernStyle.Neutral : ModernStyle.Card);
                ((RectTransform)cbtn.transform).anchoredPosition = new Vector2(CX, CY + 40 + n * 55);
            }
            ...
            {
                var i = entries[n];
                var card = PanelRect("Item" + n, root, CX + 220 + n % cols * 350, CY + 40 + n / cols * 230, 330, 210);
                ...
                ((RectTransform)card.GetChild(0)).sizeDelta = new Vector2(300, 30); ((RectTransform)card.GetChild(0)).anchoredPosition = new Vector2(16, -18);
                MLabel("LV " + i.Level + "  ·  " + (i.Credits > 0 ? i.Credits + " Credits" : i.Money + " Money"), card, 14, ModernStyle.Muted, TextAnchor.MiddleLeft);
                ((RectTransform)card.GetChild(1)).sizeDelta = new Vector2(300, 28); ((RectTransform)card.GetChild(1)).anchoredPosition = new Vector2(16, -50);
                MLabel(i.Slot == "Weapon" ? ("DMG " + i.Damage + " · int " + i.Interval + "ms") : ("PROT " + i.Protection + " · AGI-PEN " + i.AgilityPenalty), card, 13, ModernStyle.Muted, TextAnchor.MiddleLeft);
                ((RectTransform)card.GetChild(2)).sizeDelta = new Vector2(300, 26); ((RectTransform)card.GetChild(2)).anchoredPosition = new Vector2(16, -82);
                MButton("BUY", card, 16, -150, 120, 42, () => StartCoroutine(Send(Intent("Buy", i.Id))), i.Credits > 0 ? ModernStyle.Gold : ModernStyle.Blue);
            }
        }
        ...
        void RenderAmmoModern(RectTransform root)
        {
            ...
            ((RectTransform)root.GetChild(root.childCount - 1)).anchoredPosition = new Vector2(CX, MY - 100);
            if (club == null || club.BbCatalog == null) return;

            for (int n = 0; n < club.BbCatalog.Length; n++)
            {
                var b = club.BbCatalog[n];
                var card = PanelRect("Bal" + n, root, CX, CY + 40 + n * 110, 810, 96);
        ...
        void RenderOpponentsModern(RectTransform root)
        {
            ...
            {
                var m = modes[n];
                MButton(m.ToUpperInvariant(), root, CX + n * 130, MY - 95, 120, 40, () => mode = m, mode == m ? ModernStyle.Neutral : ModernStyle.Card);
                ((RectTransform)root.GetChild(root.childCount - 1)).anchoredPosition = new Vector2(CX + n * 130, MY - 95);
            }
            ...
            if (rivals == null || rivals.Length == 0)
            {
                ...
                rt.sizeDelta = new Vector2(600, 40); rt.anchoredPosition = new Vector2(CX + 300, CY + 200);
                return;
            }

            // Opponent list (left)
            var list = PanelRect("Rivals", root, CX, CY, 700, CH);
            ...
            {
                var r = rivals[n];
                var card = PanelRect("Rival" + n, list, 15, -18 - n * 130, 660, 116);
            ...
            // Detail (right)
            var det = PanelRect("OppDetail", root, CX + 720, CY, 590, 420);
            det.gameObject.AddComponent<Image>().color = ModernStyle.Panel;
            det.GetComponent<Image>().raycastTarget = false;
        ...
        void RenderHistoryModern(RectTransform root)
        {
            ...
            for (int n = 0; n < club.History.Length; n++)
            {
                ...
                var card = PanelRect("Rec" + n, root, CX, MY - 100 - n * 90, CW, 76);
                card.gameObject.AddComponent<Image>().color = ModernStyle.Card;
                card.GetComponent<Image>().raycastTarget = false;
                ...
                ((RectTransform)card.GetChild(0)).sizeDelta = new Vector2(260, 28); ((RectTransform)card.GetChild(0)).anchoredPosition = new Vector2(18, -14);
                MLabel(h.Outcome + "  ·  Money +" + h.Money + "  ·  Rating " + (h.RatingKnown ? h.RatingDelta.ToString("+0;-0;0") : "unknown"), card, 14, ModernStyle.Ink, TextAnchor.MiddleLeft);
                ((RectTransform)card.GetChild(1)).sizeDelta = new Vector2(500, 26); ((RectTransform)card.GetChild(1)).anchoredPosition = new Vector2(330, -14);
                MButton("REVIEW", card, CW - 170, -16, 130, 42, () => StartCoroutine(LoadMatch(h.Id)), ModernStyle.Blue);
            }
        }
